...
export async function renderExperienceScorecard(db, url) {
  ...
  // Honest pre-activation state (never a crash): the Pulse table may not exist
  // in this environment yet. There is no equivalent notice for written
  // Feedback - feedback_submissions is a required dependency, so its absence is
  // a real fault that surfaces as an error rather than an "inactive" banner.
